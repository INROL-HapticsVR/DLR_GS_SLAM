...
class Tracker(SLAMParameters):
    ...
    def tracking(self):
        ...
        while ii < 2000-1:
            ...
            current_image, depth_image = self.get_sharedmemory()
            ...
            if self.iteration_images == 0:
                ...
                # Update Camera pose #
                current_pose = np.linalg.inv(current_pose)
                shm = self.shared_memories[2]
                if shm is None:
                    raise RuntimeError(f"Shared memory for camera is not available.")
                buffer = shm.buf

                tmp = np.frombuffer(buffer, dtype=np.float32, count = 7)
                print (tmp)
                T = tmp[0:3]
                print (T)
                Quater = tmp[3:7]
    ...
    #             if i == 0:
    #                 # RGB 데이터와 Depth 데이터 분리
    #                 r = np.frombuffer(buffer, dtype=np.uint8, count=HEIGHT * WIDTH * 4)[0::4].reshape((HEIGHT, WIDTH))
    #                 g = np.frombuffer(buffer, dtype=np.uint8, count=HEIGHT * WIDTH * 4)[1::4].reshape((HEIGHT, WIDTH))
    #                 b = np.frombuffer(buffer, dtype=np.uint8, count=HEIGHT * WIDTH * 4)[2::4].reshape((HEIGHT, WIDTH))
    #                 # a = np.frombuffer(buffer, dtype=np.uint8, count=HEIGHT * WIDTH * 4)[3::4].reshape((HEIGHT, WIDTH))

    #                 # BGR 이미지 생성
    #                 rgb_image = np.stack([b, g, r], axis=-1)  # OpenCV는 BGR 순서 사용
    #                 cv2.imshow(f"Camera {i} - RGB", rgb_image)

    #             elif i == 1:
    #                 # depth 이미지 데이터 타입 변환
    #                 depth_image = np.frombuffer(buffer, dtype=np.float32, count=HEIGHT * WIDTH).reshape((HEIGHT, WIDTH))
    #                 depth_image = depth_image.astype(np.uint8)
    #                 cv2.imshow(f"Camera {i} - Depth", depth_image)
    ...
    def get_sharedmemory(self):
        for i in range(CAMERA_COUNT):
            ...
            if i == 0:
            # RGB 데이터와 Depth 데이터 분리
                r = np.frombuffer(buffer, dtype=np.uint8, count=HEIGHT * WIDTH * 3)[0::3].reshape((HEIGHT, WIDTH))
                g = np.frombuffer(buffer, dtype=np.uint8, count=HEIGHT * WIDTH * 3)[1::3].reshape((HEIGHT, WIDTH))
                b = np.frombuffer(buffer, dtype=np.uint8, count=HEIGHT * WIDTH * 3)[2::3].reshape((HEIGHT, WIDTH))
                # a = np.frombuffer(buffer, dtype=np.uint8, count=HEIGHT * WIDTH * 4)[3::4].reshape((HEIGHT, WIDTH))
